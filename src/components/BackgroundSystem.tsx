...
const BackgroundSystem: React.FC = () => {
  ...
  // Dynamic parallax with mobile constraints and reduced motion gating
  const yBg = useTransform(scrollY, (value) => {
    if (prefersReducedMotion) return 0;
    const progress = Math.min(Math.max(value / 8000, 0), 1);
    const maxShift = isMobile ? -75 : -350;
    return progress * maxShift;
  });

  const scaleBg = useTransform(scrollY, (value) => {
    if (prefersReducedMotion || isMobile) return 1.0;
    const progress = Math.min(Math.max(value / 8000, 0), 1);
    return 1.0 + progress * 0.12;
  });

  // Raw motion values — never trigger React re-renders
  const rawX = useMotionValue(-600);
  const rawY = useMotionValue(-600);

  // Spring-smoothed values feed directly into the element's style — GPU only
  const blobX = useSpring(rawX, { damping: 30, stiffness: 80, mass: 1 });
  const blobY = useSpring(rawY, { damping: 30, stiffness: 80, mass: 1 });
  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden pointer-events-none bg-background">
      ...
      {/* Topographical Lines — dynamic parallax with will-change */}
      <motion.div
      ...
      {/* Cursor-reactive blob — motion values → GPU composited, zero React re-renders */}
      {!isMobile && (
        <motion.div
          ...
          style={{ x: blobX, y: blobY, willChange: 'transform' }}
        />
      )}
    </div>
  );
